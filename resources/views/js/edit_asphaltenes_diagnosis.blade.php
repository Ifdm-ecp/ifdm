  ...
  */
  function switchTab() {
    ...
    function plot_asphaltene_table() {
        var evt = window.event || arguments.callee.caller.arguments[0];
        evt.preventDefault();
        data = order_matrix(clean_table_data("asphaltenes_table"));
        var pressure = [];
    ...
    $(document).ready(function() {
        ...
        $('.save_table').on('click', function() {
            ...
            //Validar datos tabla asfaltenos (Mayores a cero)
            validate_asphaltenes_data(asphaltenes_table);
        ...
        var asphaltenes_d_diagnosis_id = <?php 
            if($asphaltenes_d_diagnosis){
                echo json_encode($asphaltenes_d_diagnosis->id);
            }else{
                echo json_encode(null);
            }
        ?>;
        ...
        if (aux_hot_asphaltenes_table !== '') {//Si se han ingresado datos en la tabla volver a cargarlos
            ...
            hot_asphaltenes_table.render();
        }else{//Si es la primera vez que se ingresa a la interfaz, cargar datos de tabla asfaltenos en BD
            var aux_asphaltenes_table = [];
            $.get("{!! url('get_asphaltenes_table') !!}", {
                asphaltenes_d_diagnosis_id: asphaltenes_d_diagnosis_id
            }, function (data) {
                $.each(data, function(index, value)
                {
                    var temp_asphaltenes_table = [];
                    temp_asphaltenes_table.push(value.pressure);
                    temp_asphaltenes_table.push(value.asphaltene_soluble_fraction);
        ...
        if (aux_hot_pvt_table !== '') {//Si se han ingresado datos en la tabla volver a cargarlos
            hot_pvt_table.updateSettings({
                data: JSON.parse(aux_hot_pvt_table),
                stretchH: 'all'
            });
            hot_pvt_table.render();
        }else{//Si es la primera vez que se ingresa a la interfaz, cargar datos de tabla PVT en BD
            var aux_pvt_table = [];
            $.get("{!! url('get_pvt_table') !!}", {
                asphaltenes_d_diagnosis_id: asphaltenes_d_diagnosis_id
            }, function (data) {
                $.each(data, function(index, value)
                {
                    var temp_pvt_table = [];
                    temp_pvt_table.push(value.pressure);
                    temp_pvt_table.push(value.density);
                    temp_pvt_table.push(value.oil_viscosity);
                    temp_pvt_table.push(value.oil_formation_volume_factor);
        ...
        if (aux_hot_historical_table !== '') {//Si se han ingresado datos en la tabla volver a cargarlos
            hot_historical_table.updateSettings({
                data: JSON.parse(aux_hot_historical_table),
                stretchH: 'all'
            });
            hot_historical_table.render();
        }else{//Si es la primera vez que se ingresa a la interfaz, cargar datos de tabla historicos en BD
            var aux_historical_table = [];
            $.get("{!! url('get_historical_table') !!}", {
                asphaltenes_d_diagnosis_id: asphaltenes_d_diagnosis_id
            }, function (data) {
                $.each(data, function(index, value)
                {
                    ...
                    temp_historical_table.push(value.asphaltenes);
