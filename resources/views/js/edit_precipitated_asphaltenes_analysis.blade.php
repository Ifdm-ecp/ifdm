...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("#elemental_data_selector").bind('init change', function () { 
            if(this.checked){$(".elemental_data").prop('disabled', false);}else{$(".elemental_data").prop('disabled', true);};
        }).trigger('init');
        ...
        $('.import-components-data').on('click',function(){
            ...
            if(select_components === null){
                ...
                $.get("{!! url('import_components_data') !!}", {
                    ...
                    //En caso de que se escoja el componente Plus+, se debe cargar toda la fila en nulos
                    //La BD precargada no tiene valores para la fila de Plus +
        ...
        var edit_col_values_binary_interaction = [];

        edit_item_binary_interaction = {}
        edit_item_binary_interaction ["title"] = "Components";
        edit_item_binary_interaction ["data"] = "components";
        edit_item_binary_interaction ["type"] = "text";
        edit_item_binary_interaction ["readOnly"] = true;
        ...
        var getValueField = $.get("{{ url('get_precipitated_analysis_components') }}", { asphaltenes_d_precipitated_analysis_id: asphaltenes_d_precipitated_analysis_id }, function (data) {
            ...
            if (aux_components_table === '') {//Cargar datos desde tabla en BD
                ...
                setTimeout(function() {
                    $("#components").selectpicker('val', edit_components);
                }, 2000);
        ...
        aux_binary_interaction_coefficients_table = $("#value_binary_interaction_coefficients_table").val();
        
        if (aux_components_table === '') {//Si se hizo el modulo de estabilidad importar datos de tabla componentes. Cargar valores en columnas.
            var getValue = $.get("{{url('get_precipitated_analysis_binary_coefficients')}}", { scenario_id: asphaltenes_d_precipitated_analysis_scenario_id}, function (data) {
                $.each(data, function(index,value){
                    hot_coefficients_table.updateSettings({
                        columns: edit_col_values_binary_interaction,
            ...
            if(aux_components){
                ...
                //Armar datos de tabla con valores de compoentes
        ...
        var table_bubble_point = [];

        aux_bubble_point_table = $("#value_bubble_point_table").val();
        var asphaltenes_d_precipitated_analysis_id = {!! $asphaltenes_d_precipitated_analysis->id !!};
        
        if (aux_bubble_point_table === '') {//Cargar datos desde tabla en BD
            var getValue = $.get("{{url('get_bubble_point_data')}}", { asphaltenes_d_precipitated_analysis_id: asphaltenes_d_precipitated_analysis_id}, function (data) {
                $.each(data, function(index,value){
                    table_bubble_point.push(Object.values(value));
                });
            

                hot_bubble_point_table.updateSettings({
                    data: table_bubble_point,
                    stretchH: 'all'
                });
            });
        }else{//Si se han ingresado datos en la tabla volver a cargarlos
            hot_bubble_point_table.updateSettings({
...
$('.calculate-binary-interaction-coefficients').on('click',function(){
    ...
    if(select_components === null){
        ...
        components_id = {"N2": 1,"CO2": 2,"H2S": 3,"C1": 4,"C2": 5,"C3": 6,"IC4": 7,"NC4": 8,"IC5": 9,"NC5": 10,"NC6": 11,"NC7": 12,"NC8": 13,"NC9": 14,"NC10": 15,"NC11": 16,"NC12": 17,"NC13": 18,"NC14": 19,"NC15": 20,"NC16": 21,"NC17": 22,"NC18": 23,"NC19": 24,"NC20": 25,"NC21": 26,"NC22": 27,"NC23": 28,"NC24": 29};
...
$('.characterize_plus_component').on('click',function(){
    ...
    if(mw && sg && tb && zi_plus_data && !isNaN(mw) && !isNaN(sg) && !isNaN(tb) && !isNaN(mw) && !isNaN(zi_plus_data) && correlation_select!=' '){
        ...
        alert("Plus characterization: Missing data. Please complete the data and try again.");
    }
    ...
    function validate_binary_coefficients_data(binary_interaction_coefficients_data)
    {
        ...
        for (var i = 0; i < binary_interaction_coefficients_data.length; i++) 
        {
            ...
            {
                console.log(binary_interaction_coefficients_data[i][j]);
