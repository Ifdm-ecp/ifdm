...
<script type="text/javascript">
    //Creacion de grafico de acuerdo a valores ingresados en tabla presion de reservorio
    ...
    //Creacion de grafico de acuerdo a valores ingresados en tabla water-oil
    ...
    //Creacion de grafico de acuerdo a valores ingresados en tabla gas-oil
    ...
    //Guardar valores de select para volver a cargar en caso de error en formulario
    window.onbeforeunload = function() {
        var a = $('#field').val();
        localStorage.setItem('field', $('#field').val());
        var b = $('#wellName').val();
        localStorage.setItem('wellName', $('#wellName').val());
    }

    //Cargar valores de select en caso de que exista error en formulario
    window.onload = function() {
        ...
        var pozo = localStorage.getItem('wellName');
    ...
    $(document).ready(function() {
        $('#loading').show();
        //Cargar valores de select de acuerdo a datos de la BD
        $("#myModal").modal('show');

        var selected = [{!!$data->pozo->cuenca_id!!},{!!$data->pozo->campo_id!!},{!!$data->pozo_id!!},{!!$data->formacion_id!!},'{!!$data->nombre!!}'];
            
            setTimeout(function() {
                $.get("{{url('fieldbybasin')}}", {
                    basin: selected[0]
                },function(data) {
                    seleccionar(data, "#field", selected[1]);
                });
                setTimeout(function() {
                    $.get("{{url('wellbyfield')}}", {
                        field: selected[1]
                    },function(data) {
                        seleccionar(data, "#wellName", selected[2]);
                    });
                    setTimeout(function() {
                        $.get("{{url('formationbyfield')}}", {
                            field: selected[1]
                        },function(data) {
                            seleccionar(data, "#formacionName", selected[3]);
                        });
                        setTimeout(function() {
                            $.get("/intervalos/"+selected[3], {
                                fomracion: selected[3]
                            },function(data) {
                                seleccionar2(data, "#nameInterval",selected[4]);
                            });
                            $('#loading').hide();
                        }, 400);
                    }, 400);
    ...
    $('#field').change(function() {
        ...
        $.get("{{url('formationbyfield')}}", {
            ...
            seleccionar(data, "#formacionName", '');
        });

        $.get("{{url('wellbyfield')}}", {
            field: dato
        },function(data) {
            seleccionar(data, "#wellName", '');
        });
    });

    $("#formacionName").change(function() {
        var dato = $("#formacionName").val();
        $.get("/intervalos/"+dato, {
            basin: dato
        },function(data) {
            console.log(data)
            seleccionar2(data, "#nameInterval", '');
        });
    });

    function seleccionar(data, elemento, selected )
    {
        $(elemento).empty();
        $.each(data, function(index, value) {
    ...
    function seleccionar2(data, elemento, selected )
    {
        $(elemento).empty();
        $.each(data, function(index, value) {
    ...
    //Creacion y carga de datos para tablas - Consultas a la BD para traer valores guargados
    var reservoir = [];
    $.get("{!! url('excel_reservoir_interval') !!}", {
        formacion: <?php echo $data->id ?>
    }, function(data) {
        $.each(data, function(index, value) {
            var reservoir_p = [];
            reservoir_p.push(value.fecha);
            reservoir_p.push(value.valor);
            reservoir_p.push(value.comentario);
            reservoir.push(reservoir_p);

        });

        if ($("#reservoirPressure_table").val() != '') {
            reservoir = $("#reservoirPressure_table").val();
            if (reservoir === '') {
                reservoir = [
                [, ],
                [, ],
    ...
    //Funcion para validar valores de tablas y formulario. Mostrar ventana modal con errores y advertencias en caso de que sea necesirio
    function Mostrar() {
        ...
        //Validacion de rangos normales en inputs de formulario
        ...
        //Cambio de color en input y linea en caso de que el valor esta fuera de rangos normales
        if ($('#reservoir').val() == '') {
        ...
        //Creacion de variables para mostrar errores y advertencias
        if (!valid_GL || !valid_WO|| !valid_GL0 || !valid_WO0 || !valid_reservoir || !valid_sw || !valid_krw || !valid_kro || !valid_sg || !valid_krg || !valid_krl || !valid_pcwo || !valid_pcgl || !valid_pressure) {
            ...
            //Creacion de ventana modal para mostrar errores y advertencias de valores fuera de rango normal
