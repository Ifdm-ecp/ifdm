...
<script type="text/javascript">
  ...
  function plot_pvt_oil()
  {
    ...
    var data = $("#excel_table").handsontable('getData');
    var pressure = [];
    var oil_viscosity = [];
  ...
  function defineView_reload(fluido)
  {
    console.log(typeof(fluido));
     if(fluido == "1")
     {
  ...
  function enviar(validate)
  {
    ...
    //Oil
    //var datos_unidades_hidraulicas = order_pvt(curr_hot.getData());
    var datos_unidades_hidraulicas = order_pvt($("#excel_table").handsontable('getData'));
    var datos_wateroil = $("#excelwateroil").handsontable('getData');
    var datos_gasliquid = $("#excelgasliquid").handsontable('getData');

    //Gas
    var datos_pvt_ipr_g = order_pvt($("#excel_tabular_pvt_fluid_g").handsontable('getData'));
    ...
    //console.log(datos_unidades_hidraulicas);
    //console.log(datos_wateroil);
    //console.log(datos_gasliquid);

    //console.log(datos_pvt_ipr_g);

    //console.log(pvt_cg_data);
    //console.log(go_cg_data);
    //console.log(dropout_cg_data);
    ...
    if($("#fluido").val()=="1")
    {
      ...
      if($("#check_tabular_rel_perm_o").is(":checked"))
      {
              ...
              {
                 valid1=false;
                 mensaje="The first Gas-Oil krg value must be 0. Check your Relative Permeability Data";
              }
              if(((d0 ==0) && (d1==0) && (d2==0)) || ((d3==0) && (d4==0) && (d5==0)))
              {
                 valid1=false;
      ...
      for (var i = 0; i<datos_unidades_hidraulicas.length; i++)
      {
         ...
         if((d0 != null && d0!="") || (d1 != null && d1!="") || (d2 != null && d2!="") || (d3 != null && d3!=""))
         {
            ...
            {
               valid2=false;
               mensaje = "Your PVT data must be greather than 0";
            }
            if(d0<14.7)
            {
               valid2=false;
      ...
      valid = valid1 && valid2 && valid3 && valid4 && valid5 && valid10;
    }
    else if($("#fluido").val()=="2")
    {
    ...
    else if($("#fluido").val()=="3")
    {
      ...
      else
      {
         ...
         {
            valid1_cg =false;
            mensaje="The last Krog data must be 0 at Gas Oil data. Check your Relative Permeability Data";
         }
      }
      ...
      {
         valid2_cg =false;
         mensaje="Check your PVT pressure data. You must include the saturation pressure, a higher value, and a lower value.";
      }
    ...
    //Verifica si las validaciones a ordenamiento y rangos se hicieron correctamente antes de enviar el controlador
    if(!valid)
    {
      evt.preventDefault(); 
      alert(mensaje);
  ...
  //defineView_reload($("#fluid_select_value").val());
...
/*
  //Tooltips
  $("#check_tabular_fluid_o").tooltip({
    title : 'Please insert at least two complete rows. \n All data must be positive.\n Pressure must be at least 14.7 psi.\n In the pressure column must be contained the Saturation Pressure, a lower value and a higher value '  // title option passed via javascript
  });

  $("#check_tabular_fluid_g").tooltip({
    title : 'Please insert at least two complete rows. All data must be positive. Pressure must be at least 14.7 psi. In the pressure column must be contained the Saturation Pressure, a lower value and ahigher value. '  // title option passed via javascript
  });

  */
  $("#check_tabular_rel_perm_o").tooltip({
    title : 'Please insert at least two complete rows. All data must be positive. No row must be completely 0. Sw, Krw, Kro and Sg, Krg, Krog values must be between 0 and 1. Sw,Krw and Sg,Krg must beascending sorted and Kro and Krog must be descending sorted. The first Krw must be 0. The last Kro and Krog data must be 0'
  });


  /* *********Sección de control de eventos ********* */

  //Visibilidad del botón Run IPR
  ...
  //Gas-Oil / Water-Oil - Oil
  var data_aux_wo = [];
  $.get("{!!url('initial_wo_ipr')!!}",
  {
    formation:{!! $scenary_s->formacion_id !!},
    well:{!! $scenary_s->pozo_id !!}
  },
  function(data)
  {
    $.each(data, function(index, value)
    {
      ...
      row.push(value.krw);
      row.push(value.kro);
      data_aux_wo.push(row);
    });

  if($("#wateroil_hidden").val()!='')
  {
    data_aux_wo = $("#wateroil_hidden").val();
    data_aux_wo =  JSON.parse(data_aux_wo);
  }
  else
  {
    if(data_aux_wo.length==0)
    {
      data_aux_wo = [[,,],[,,],[,,],[,,],[,,]];
    }
  }
...
}, function(data)
{
  $.each(data, function(index,value)
  {
    ...
    row.push(value.krg);
    row.push(value.krl);
    data_aux_gl.push(row);
  });

  if($("#gasliquid_hidden").val()!='')
  {
    data_aux_gl = $("#gasliquid_hidden").val();
    data_aux_gl =  JSON.parse(data_aux_gl);
  }
  else
  {
    if(data_aux_gl.length==0)
    {
      data_aux_gl = [[,,],[,,],[,,],[,,],[,,]];
    }
  }
...
$excel_gas_oil_g = $('#excel_gas_oil_g');

$excel_gas_water_g = $('#excel_gas_water_g');

$excel_gas_oil_g.handsontable({
   //data: [[0,0,0]], 
   colWidths: [80, 80, 80] ,
   rowHeaders: true, 
       columns: [
...
$excel_gas_water_g.handsontable({
   //data: [[0,0,0]], 
   colWidths: [80, 80, 80] ,
   rowHeaders: true, 
       columns: [
     ...
     {title:"Sw", data: 0,type: 'numeric', format: '0[.]0000000'},
     {title:"Krg",data: 1,type: 'numeric', format: '0[.]0000000'},
     {title:"Krw", data: 2,type: 'numeric', format: '0[.]0000000'}
   ],
   minSpareRows: 1,
   contextMenu: true,
});
...
}, function(data)
{
  $.each(data, function(index,value)
  {
    ...
    row.push(value.krg);
    row.push(value.krl);
    data_gasoil_cg_aux.push(row);
  });

  if($("#gas_oil_kr_cg").val()!='')
  {
    data_gasoil_cg_aux = $("#gas_oil_kr_cg").val();
    data_gasoil_cg_aux =  JSON.parse(data_gasoil_cg_aux);
  }
  else
  {
    if(data_gasoil_cg_aux.length==0)
    {
      data_gasoil_cg_aux = [[,,],[,,],[,,],[,,],[,,]];
    }
  }
...
//Pvt Gas
var data_aux_pvtG = [];
$.get("{!! url('initial_pvt_ipr') !!}",
{
  field: {!! $formacion->formacion_id !!}
}, function(data)
{
  $.each(data, function(index,value)
  {
      var row = [];
      row.push(value.pressure);
      row.push(value.ug);
      row.push(value.bg);
      data_aux_pvtG.push(row);
  });
  console.log("A "+data_aux_pvtG.length);

  if($("#pvt_gas_ipr").val()!='')
  {
    data_aux_pvtG = $("#pvt_gas_ipr").val();
    data_aux_pvtG =  JSON.parse(data_aux_pvtG);
  }
  else
  {
    if(data_aux_pvtG.length==0)
    {
      data_aux_pvtG = [[,,,,],[,,,,],[,,,,],[,,,,],[,,,,]];
    }
  }
...
}, function(data)
{
  $.each(data, function(index,value)
  {
      ...
      row.push(value.ug);
      row.push(value.rv);
      data_aux_pvt_c_g.push(row);
  });
  console.log("A "+data_aux_pvt_c_g.length);

  if($("#pvt_cg").val()!='')
  {
    data_aux_pvt_c_g = $("#pvt_cg").val();
    data_aux_pvt_c_g =  JSON.parse(data_aux_pvt_c_g);
  }
  else
  {
    if(data_aux_pvt_c_g.length==0)
    {
      data_aux_pvt_c_g = [[,,,,],[,,,,],[,,,,],[,,,,],[,,,,]];
    }
  }

$excel_tabular_pvt_fluid_c_g = $('#pvt_c_g');
$excel_tabular_pvt_fluid_c_g.handsontable({
   data: data_aux_pvt_c_g, 
...
if($("#dropout_cg").val()!='')
{
  data_dropout_cg_aux = $("#dropout_cg").val();
  if(data_dropout_cg_aux==='')
  {
    data_dropout_cg_aux = [[,],[,],[,],[,],[,]];
  }
  else
  {
    data_dropout_cg_aux =  JSON.parse(data_dropout_cg_aux);
  }
}
else
{
...
}, function(data)
{
  ...
  if($("#presiones_table").val()!='')
  {
    console.log("if2");
    data_aux = $("#presiones_table").val();
...
/* *********Sección de validación para pestañas ********* */
//Cambio de color a pestañas reload
...
if(fluido=="1")
{
  ...
  //var datos_unidades_hidraulicas = order_pvt(curr_hot.getData());
  var datos_unidades_hidraulicas = [[,,,,],[,,,,],[,,,,],[,,,,],[,,,,]];
...
$(".nav.nav-tabs li").on("click", function()
{
    ...
    if(fluido=="1")
    {
      //Lectura de datos de las tablas del formulario
      //Oil
      var datos_unidades_hidraulicas = $("#excel_table").handsontable('getData');
      var datos_wateroil = $("#excelwateroil").handsontable('getData');
      var datos_gasliquid = $("#excelgasliquid").handsontable('getData');
